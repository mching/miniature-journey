...
The podcast I listen to said something like, "I bet Justin Smoak must have one of the lowest BABIPs of anyone to this point in his career," and I thought it might be a good chance to explore this nebulous BABIP a little.
...
calculation of career AB to that point of the career. 

```{r}
Batting %>% filter(yearID > 1953) %>% 
  group_by(playerID) %>% 
  mutate(careerAB = cumsum(AB)) %>% 
  select(yearID, playerID, careerAB) %>% 
  filter(playerID == "aaronha01")
```
