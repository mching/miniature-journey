...
There were `r length(x$class)` students represented. There were `r sum(x$gender == "f")` females and `r sum(x$gender == "m")` males. 

Here was the distribution of students by number of years attended. There were `r sum(x$years == 13)` students who attended the school from kindergarten through graduation (13 years).
...
To determine if these was a significant difference, I performed a Mann Whitney U test (aka Wilcoxon rank sum test). The null hypothesis was that there was no difference in the distribution of the two groups of students. The resulting p-value was `r round(wilcox.test(x$lists~x$sdoi)$p.value, 3)`. I was not able to reject the null hypothesis. There did not appear to be a significant difference in the achievement between the two groups.
